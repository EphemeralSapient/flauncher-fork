...
class AppsService extends ChangeNotifier {
      ...
      switch (event["action"]) {
        ...
        case "PACKAGE_CHANGED":
          await _database.persistApps([
            _buildAppCompanion(event["activitiyInfo"]),
          ]);
          break;
  ...
  Future<void> _initDefaultCategories() => _database.transaction(() async {
    final tvApplications = _applications.where(
      (element) => element.sideloaded == false,
    );
    final nonTvApplications = _applications.where(
      (element) => element.sideloaded == true,
    );
    if (tvApplications.isNotEmpty) {
      ...
      final tvAppsCategory = _categoriesWithApps
          .map((e) => e.category)
          .firstWhere((element) => element.name == "TV Applications");
    ...
    if (nonTvApplications.isNotEmpty) {
      await addCategory("Non-TV Applications", shouldNotifyListeners: false);
      final nonTvAppsCategory = _categoriesWithApps
          .map((e) => e.category)
          .firstWhere((element) => element.name == "Non-TV Applications");
  ...
  Future<void> _refreshState({bool shouldNotifyListeners = true}) async {
    await _database.transaction(() async {
      ...
      final appsRemovedFromSystem =
          (await _database.listApplications())
              .where(
                (app) =>
                    !appsFromSystem.any(
                      (systemApp) =>
                          systemApp.packageName.value == app.packageName,
                    ),
              )
